...
function typescript(options = {}) {
    ...
    const createProgram = (context) => createWatchProgram(ts, context, {
        formatHost,
        resolveModule,
        parsedOptions,
    ...
    return {
        ...
        buildStart(rollupOptions) {
            ...
            if (Array.isArray(rollupOptions.input)) {
                parsedOptions.fileNames = rollupOptions.input.map((fileName) => path__namespace.resolve(fileName));
            }
        ...
        async load(id) {
            if (!filter(id))
                return null;
            const resolvedId = path__namespace.resolve(id);
            ...
            if (!parsedOptions.fileNames.includes(fileName)) {
                ...
                parsedOptions.fileNames.push(path__namespace.resolve(fileName));
